...
# when this tracker first observes the pane as working. The timing helper keeps
# stopped durations visible and resumes blocked turns without counting wait time.
set -eu
...
# Stateful timing is separate from profile detection: keep $elapsed on blocked,
# done and seen/idle agents, and migrate any in-flight legacy .starts values.
printf '%s' "$agents" |
  /opt/homebrew/bin/python3 "$config_dir/agent-turn-timer.py" "$state_file"
